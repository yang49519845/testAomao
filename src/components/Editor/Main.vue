...
import { cards, plugins, pluginConfig, onLoad, doc } from "./config";
import { getDocValue, setDocValue } from "../../utils";

import * as Y from 'yjs';
import { WebsocketProvider } from '@aomao/plugin-yjs-websocket';
import { YjsEditor, withYjs } from "../../plugins/yjs/src/index";
import { testYDoc } from "./testYDoc";
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if (container.value) {
        ...
        if (provider.value) {
          ...
          provider.value.on('customMessage', handleCustomMessage);
          const sharedType = doc.get('demo', Y.XmlElement) as Y.XmlElement;

          withYjs(engineInstance, sharedType, provider.value.awareness, {
            data: { name: uuid(), color: 'red' },
          });
        }

        if (YjsEditor.isYjsEditor(engineInstance)) {
          YjsEditor.connect(engineInstance)
        }
    ...
    onUnmounted(() => {
      ...
      // if(engine.value && YjsEditor.isYjsEditor(engine.value)) {
      //   YjsEditor.disconnect(engine.value);
      // }
    });

    watchEffect(() => {
      if (typeof window !== 'undefined') {
        provider.value = new WebsocketProvider('ws:localhost:3000/', 'demo', doc);
        ...
        if (provider.value) provider.value.on('status', handleStatus);
      }
    })
